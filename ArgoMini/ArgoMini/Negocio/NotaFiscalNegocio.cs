...
namespace ArgoMini.Negocio
{
    public class NotaFiscalNegocio
    {
        ...
        private string MontarXmlNfce()
        {
            ...
            var avulsa = string.Empty;
            var retirada = string.Empty;
            var entrega = string.Empty;
            var cobranca = string.Empty;
            var exportacao = string.Empty;
            var compra = string.Empty;
            var detalhamentoCanaAcucar = string.Empty;
            var autorizadosDownloadXml = string.Empty;

            var xml = _nfeUtil.NFe310(versao, chave, identificador, emitente, avulsa, destinatario, retirada, entrega, detalhesItens, totais, transporte, cobranca, pagamento, informacoesAdicionais, exportacao, compra, detalhamentoCanaAcucar, autorizadosDownloadXml);
            ...
            const string versaoQrCode = "2"; // 100 para NFC-e até a v 3.10 | 2 para v 4.0 ou acima.
            var urlConsulta = string.Empty;
            const string indSinc = "1"; // Indicador de processamento síncrono (0=não ou 1=sim), este parâmetro será utilizado para criar o lote de envio.
        ...
        private string ConverterParaChave()
        {
            ...
            var ano = "18";
            var mes = "10";
            var cnpj = "02807779000187";
            var modelo = "65";
            var serie = "112";
            var numero = "9";
            var tpEmis = "1";
            var codigoSeguranca = "ARGOSIS"; //"ARGOMINI";
        ...
        private string ConverteParaIdentificador()
        {
            ...
            var verProc = "000";
            var dhCont = string.Empty;
            var xJust = string.Empty;

            return _nfeUtil.identificador400(cUf, cNf, natOp, mod, serie, nNf, dhEmi, dhSaiEnt, tpNf, idDest, cMunFg, nfRef, tpImp, tpEmis, cDv, tpAmb, finNfe, indFinal, indPres, procEmi, verProc, dhCont, xJust);
        ...
        private string converteEmitente()
        {
            ...
            var xNome = "Ivone Danieli Savian"; // 60 caracteres
            var xFant = "Minimercado Dona De Casa"; //60 caracteres
            var xLgr = "VISCONDE DE TAUNAY";
            var nro = "132";
            var xCpl = string.Empty;
            var xBairro = "RIO BRANCO";
            var cMun = "4305108";
            var xMun = "Caxias Do Sul";
            var uf = "RS";
            var cep = "95097730";
            var cPais = "1058";
            var xPais = "Brasil";
        ...
        public string ConverterParaItens()
        {
            ...
            var produto = ConverterParaDetalhesProduto((nItem == 1 && ambienteHomologacao), ambienteHomologacao);
            var imposto = ConverterParaImpostosProduto();
            ...
            //foreach (var item in _notaSaida.Items)
            //{
        ...
        protected string ConverterParaDetalhesProduto(bool utilizarDescricaoHomologacao, bool ambienteHomologacao = false)
        {

            var cProd = "7894900700015"; // codigo barras
            var cEan = "SEM GTIN";
            var xProd = utilizarDescricaoHomologacao ? @"NOTA FISCAL EMITIDA EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL" : "coca cola lata";
            var ncm = "22021000";
            var nveOpc = string.Empty;
            var cestOpc = "0301100"; // cest mercadoria || string.empty
            var indEscalaOpc = string.Empty;
            var cnpjFabOpc = string.Empty;
            var cBenefOpc = string.Empty;
            var exitipi = string.Empty;
            var cfop = 5405;
            var uCom = "UN";
            var qCom = "1"; //quantidade replace , por . toString
            var vUnCom = "3.99";
            var vProd = 3.99; // precisa ser o total bruto
            var cEanTrib = "SEM GTIN";
            var uTrib = "UN";
            var qTrib = "1";
            var vUntrib = "3.99";
            var vFrete = 0;
            ...
            var indTot = 1;
            var di = string.Empty;
            var detExportacaoOpc = string.Empty;
            var detEspecifico = string.Empty;
            var xPed = string.Empty;
            var nItemPed = string.Empty; // Randon precisa que vá esse código 10. Se futuramente precisar usar esta informação de forma diferente precisamos prever no cadastro
            var nFciOpc = string.Empty;
            var rastroOpc = string.Empty;

            return _nfeUtil.produto400(cProd, cEan, xProd, ncm, nveOpc, cestOpc, indEscalaOpc, cnpjFabOpc, cBenefOpc, exitipi, cfop, uCom, qCom, vUnCom, vProd, cEanTrib, uTrib, qTrib, vUntrib, vFrete, vSeg, vDesc, vOutro, indTot, di, detExportacaoOpc, detEspecifico, xPed, nItemPed, nFciOpc, rastroOpc);

        }

        protected string ConverterParaImpostosProduto()
        {
            var vtotTrib = 0D;
            var icms = ConverterParaIcmsProduto();
            var ipi = string.Empty;
            var ii = string.Empty;
            var pis = ConverterParaPisProduto();
            var pisSt = string.Empty; ;
            var cofins = ConverterParaCofinsProduto();
            var cofinsSt = string.Empty; ;
            var issqn = string.Empty; ;
            var icmsUfDest = string.Empty; ;

            return _nfeUtil.impostoNT2015003(vtotTrib, icms, ipi, ii, pis, pisSt, cofins, cofinsSt, issqn, icmsUfDest);
        }
        ...
        private string ConverterParaIcmsProduto()
        {
            ...
            return _nfeUtil.icms400(orig, cst, modBc, pRedBc, vBc, pIcms, vIcms, modBcSt, pMvaSt, pRedBcSt, vBcSt, pIcmsSt, vIcmsSt, vBcStRet,
                vIcmsStRet, vBcStDest, vIcmsStDest, motDesIcms, pBcOp, ufSt, pCredSn, vCredICmsSn, vICmsDeson, vImsOp, pDif, vIcmsDif,
                vBcfcp, pfcp, vfcp, vBcfcpst, pfcpst, vfcpst, vBcfcpstRet, pFcpstRet, vFcpstRet, pSt);
        }
        ...
        private string ConverterParaPisProduto()
        {
            ...
            return _nfeUtil.PIS(cst, vBc, pPis, vPis, qBcProd, vAliqProd);
        }
        ...
        private string ConverterParaCofinsProduto()
        {
            ...
            return _nfeUtil.COFINS(cst, vBc, pCofins, vCofins, qBcProd, vAliqProd);
        }
        ...
        {
            var vBc = 0;//_notaSaida.Items.Sum(p => p.BaseCalculoIcms).ToDouble();
            var vIcms = 0;//_notaSaida.Items.Sum(p => p.ValorIcms).ToDouble();
            var vBcSt = 0;//_notaSaida.Items.Sum(p => p.BaseCalculoIcmsSubstituicaoTributaria).ToDouble();
            var vSt = 0;//_notaSaida.Items.Sum(p => p.ValorIcmsSubstituicaoTributaria).ToDouble();
            var vProd = 3.99; //_notaSaida.ValorMercadorias.ToDouble() - (_notaSaida.ValorOutrasDespesasAcessorias.ToDouble() + _notaSaida.ValorFrete.ToDouble() + _notaSaida.ValorSeguro.ToDouble() + _notaSaida.ValorIpi.ToDouble() + _notaSaida.ValorIcmsSubstituicaoTributaria.ToDouble());
            var vFrete = 0;//_notaSaida.Items.Sum(p => p.ValorFrete).ToDouble();
            var vSeg = 0;//_notaSaida.Items.Sum(p => p.ValorSeguro).ToDouble();
            var vDesc = 0;//_notaSaida.Items.Sum(p => p.ValorDesconto).ToDouble();
            var vIi = 0D;
            var vIpi = 0;//_notaSaida.Items.Sum(p => p.ValorIpi).ToDouble();
            var vPis = 0;//_notaSaida.Items.Sum(p => p.ValorPis).ToDouble();
            var vCofins = 0;//_notaSaida.Items.Sum(p => p.ValorCofins).ToDouble();
            var vOutro = 0;//_notaSaida.Items.Sum(p => p.ValorOutrasDespesasAcessorias).ToDouble();
            var vNf = 3.99;//_notaSaida.ValorTotalNota.ToDouble();
            var vTotTrib = 0D;
            var vIcmsDeson = 0D;
            var vIcmsUfDest = 0;//_notaSaida.Items.Sum(p => p.ValorDifalUfDestino).ToDouble();
            var vIcmsUfRemet = 0;//_notaSaida.Items.Sum(p => p.ValorDifalUfOrigem).ToDouble();
            var vFcpUfDest = 0D;
            var vFcp = 0D;
            var vFcpst = 0D;
            var vFcpstRet = 0D;
            ...
            return _nfeUtil.totalICMS400(vBc, vIcms, vBcSt, vSt, vProd, vFrete, vSeg, vDesc, vIi, vIpi, vPis, vCofins, vOutro, vNf, vTotTrib, vIcmsDeson, vIcmsUfDest, vIcmsUfRemet, vFcpUfDest, vFcp, vFcpst, vFcpstRet, vIpiDevol);
        }
        ...
        public string ConverterParaTransporte()
        {
            ...
            var retTransp = string.Empty;
            var veicTransp = string.Empty;
            var reboque = string.Empty;
            var vagao = string.Empty;
            var balsa = string.Empty;
            var vol = string.Empty;

            return _nfeUtil.transportador2G(modFrete, transportadora, retTransp, veicTransp, reboque, vagao, balsa, vol);
        }
        ...
        private string ConverterParaDetalhesPagamento()
        {
            ...
            var tpIntegraOpc = string.Empty;
            var cnpjOpc = string.Empty;
            var tBandOpc = string.Empty;
            var cAutOpc = string.Empty;

            //testa se o pagamento foi cartão, cheque, dinheiro -- sempre será dinheiro
            var tPag = "01"; // dinheiro

            var vPag = 3.9900; // valor pago da nota

            // só terá um pagamento por nota, n precisa fazer append
            return _nfeUtil.detPag(indPagOpc, tPag, vPag, tpIntegraOpc, cnpjOpc, tBandOpc, cAutOpc);
        }
        ...
        {
            var infAdFisco = string.Empty;
            var infCpl = string.Empty;
            var obsCont = string.Empty;
            var obsFisco = string.Empty;
            var procRef = string.Empty;

            return _nfeUtil.infAdic2G(infAdFisco, infCpl, obsCont, obsFisco, procRef);
        }
