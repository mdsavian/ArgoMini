...
namespace ArgoMini.Negocio
{
    ...
    public class FlexDocsNegocio
    {
        ...
        public void SalvarXml(string nfce, int tipoNotaFiscal)
        {
            string folder = $@"c:\temp\{(tipoNotaFiscal == 1 ?"NotasFiscaisEmitida" : "NotasFiscaisImportada")}";
            string arquivo = Path.Combine(folder, "testeXml.xml"); 
        ...
        public XmlDocument BaixarXmlNfe(string chave)
        {
            ...
            try
            {
                ...
                xml.LoadXml(result);
                new FlexDocsNegocio().SalvarXml(result, 2);
            }
            catch (Exception ex)
            {
        ...
        //public void CancelarNfce(long notaSaidaId, string serie, string justificativaCancelamento)
        //{
        //    using (var contexto = ArgoMiniContext.CriarContexto())
        //    {
        //        var notaSaida = contexto.CarregarTodos<NotaFiscalSaida>().First(p => p.Id == notaSaidaId && p.Serie.Equals(serie));
        //        //var notaFiscalConsumidorEletronica = contexto.CarregarTodos<NotaFiscalConsumidorEletronica>().First(n => n.NotaFiscalSaidaId == notaSaidaId);
        //        //var configuracoes = App.ConfiguracaoDocumentoEletronico.First(k => k.Key == unidadeDeNegocioId).Value;

        //        ValidacoesCancelamento.ValidarCancelamento(notaSaida);
        //        var dataHora = NotaFiscalConsumidorEletronicaNegocio.DataEHora();

        //        var siglaWs = configuracoes.SiglaWsCupomEletronico;
        //        var tipoAmbiente = (int)configuracoes.Ambiente;
        //        var nomeCertificado = configuracoes.CertificadoDigital;
        //        var versao = configuracoes.VersaoEventoCancelamento;
        //        var chave = notaFiscalConsumidorEletronica.Chave;
        //        var nroProtocolo = notaFiscalConsumidorEletronica.AutorizacaoNumeroProtocolo;
        //        var justificativa = justificativaCancelamento;
        //        var dhEvento = dataHora.ToString("yyyy-MM-dd HH:mm:ss");
        //        var proxy = string.Empty;
        //        var funcionario = string.Empty;
        //        var senha = string.Empty;
        //        var licenca = string.Empty;

        //        var procCancNFe = _nfeUtil.CancelaNFEvento(siglaWs, tipoAmbiente, nomeCertificado, versao, out var msgDados, out var msgRetWs, out var cStat, out var msgResultado, chave, nroProtocolo, justificativa, dhEvento, out var nProtocoloCanc, out var dProtocoloCanc, proxy, funcionario, senha, licenca);

        //        // Nota Cancelada
        //        if (cStat == 135 || cStat == 155 || cStat == 573)
        //        {
        //            var reciboCancelamento = ParseNfe.ConverterParaReciboNfe(msgRetWs);
        //            new NotaFiscalSaidaNegocio().CancelarNotaSaida(contexto, notaSaida, justificativaCancelamento, EStatusDocumentoSaida.Cancelada, dataHora);
        //            new NotaFiscalConsumidorEletronicaNegocio().SalvarDadosCancelamento(contexto, notaFiscalConsumidorEletronica, reciboCancelamento, procCancNFe);
        //        }
        //        else
        //        {
        //            Log.Write.Error($"Falha ao cancelar a NFC-e {notaSaida.Numero} série {notaSaida.Serie}:{Environment.NewLine}Código falha: {cStat} - {msgResultado}");
        //            throw new Exception($"Falha no cancelamento da NFC-e: {msgResultado}");
        //        }
        //    }
        //}

        //public void InutilizarNfce(long nfceNumero, string nfceSerie, long unidadeDeNegocioId, long? caixaId, string justificativaCancelamento)
        //{
        //    using (var contexto = DbContexto.CriarContexto())
        //    {
        //        var notaSaida = contexto.CarregarTodos<NotaFiscalSaida>().FirstOrDefault(p => p.TipoDocumentoSaida == ETipoDocumentoSaida.CupomFiscalEletronico && p.Numero == nfceNumero && p.Serie == nfceSerie && p.UnidadeDeNegocioId == unidadeDeNegocioId);
        //        var notaFiscalConsumidorEletronica = notaSaida == null ? new NotaFiscalConsumidorEletronica() : contexto.CarregarTodos<NotaFiscalConsumidorEletronica>().FirstOrDefault(n => n.NotaFiscalSaidaId == notaSaida.Id && n.UnidadeDeNegocioId == unidadeDeNegocioId);
        //        var unidadeDeNegocio = contexto.CarregarTodosSomenteLeitura<UnidadeDeNegocio>().Include(u => u.Cidade.Estado).Include(u => u.Telefones).First(u => u.Id == unidadeDeNegocioId);
        //        unidadeDeNegocio.OpcoesAvancadas = contexto.CarregarTodosSomenteLeitura<OpcoesAvancadas>().First(o => o.UnidadeDeNegocioId == unidadeDeNegocio.Id);
        //        var configuracoes = App.ConfiguracaoDocumentoEletronico.First(k => k.Key == unidadeDeNegocioId).Value;

        //        this.ValidaInutilizacao(notaSaida);
        //        var dataHora = NotaFiscalConsumidorEletronicaNegocio.DataEHora();

        //        var siglaWs = configuracoes.SiglaWsCupomEletronico;
        //        var tipoAmbiente = (int)configuracoes.Ambiente;
        //        var nomeCertificado = configuracoes.CertificadoDigital;
        //        var versao = configuracoes.Versao;
        //        var cUf = unidadeDeNegocio.Cidade.Estado.Id.ToString();
        //        var ano = dataHora.ToString("yy");
        //        var cnpj = unidadeDeNegocio.Cnpj.RemoverCaracteresNaoNumericos();
        //        var modelo = "65";
        //        var serie = nfceSerie;
        //        var nroNfeInicial = nfceNumero.ToString();
        //        var nroNfeFinal = nfceNumero.ToString();
        //        var justificativa = justificativaCancelamento;
        //        var proxy = configuracoes.Proxy;
        //        var funcionario = configuracoes.UsuarioProxy;
        //        var senha = configuracoes.SenhaProxy;
        //        var licenca = configuracoes.LicencaFlexDocs;

        //        _nfeUtil.InutilizaNroNF2G(siglaWs, tipoAmbiente, nomeCertificado, versao, out var msgDados, out var msgRetWs, out var cStat, out var msgResultado, cUf, ano, cnpj, modelo, serie, nroNfeInicial, nroNfeFinal, justificativa, out var nProtocoloInut, out var dProtocoloInut, proxy, funcionario, senha, licenca);
        ...
        //            if (notaSaida == null)
        //            {
        //                notaSaida = new NotaFiscalSaida
        //                {
        //                    UnidadeDeNegocioId = unidadeDeNegocioId,
        //                    TipoDocumentoSaida = ETipoDocumentoSaida.CupomFiscalEletronico,
        //                    DataEmissao = dataHora,
        //                    HoraEmissao = dataHora.TimeOfDay,
        //                    EnderecoPaisId = 1058,
        //                    CaixaId = caixaId,
        //                    Numero = nfceNumero,
        //                    Serie = nfceSerie
        ...
        //            var reciboInutilizacao = ParseNfe.ConverterParaReciboNfe(msgRetWs);
        //            new NotaFiscalSaidaNegocio().CancelarNotaSaida(contexto, notaSaida, justificativaCancelamento, EStatusDocumentoSaida.Inutilizada, dataHora);
        //            new NotaFiscalConsumidorEletronicaNegocio().SalvarDadosInutilizacao(contexto, notaSaida, notaFiscalConsumidorEletronica, reciboInutilizacao);
        //        }
        //        else
        //        {
        //            Log.Write.Error($"Falha ao inutilizar a NFC-e {notaSaida.Numero} série {notaSaida.Serie}:{Environment.NewLine}Código falha: {cStat} - {msgResultado}");
        //            throw new Exception($"Falha na inutilização da NFC-e: {msgResultado}");
        //        }
        //    }
        //}
